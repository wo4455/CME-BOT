...
const Display = ({ userData }) => {
    ...
    const Button = ({ title, action, state, customStyles }) => {
        return (
            <li onClick={action} className={`p-6 cursor-pointer hover:bg-gray-500 transition-colors duration-300 ${customStyles}`}>
                <button
                ...
                {state && (
                    <span className={`check-mark show absolute ml-8 text-green-500 text-xl`}>&#10003;</span>
                )}
            </li> 
        );
    };
    ...
    return (
        <>
            ...
            <div className={`${userData.token ? 'fixed' : 'hidden'} bottom-8 right-8`}>
                <div onClick={() => setNavOpen(!navOpen)} className='flex items-center cursor-pointer hover:opacity-75'>
                    <p className='font-[Poppins] mr-1 text-xl tracking-wider text-dkBlue font-semibold'>MENU</p>
                ...
                {navOpen && (
                    <ul className='fixed right-8 bottom-16 w-[fit-content] h-[fit-content] bg-black bg-opacity-70 rounded-md'>
                        <Button title={'Load'} action={loadChats} state={loadCheckMark} customStyles={'border-b border-white'} />
                        <Button title={'Save'} action={saveChats} state={saveCheckMark} customStyles={'border-b border-white'} />
                        <Button title={'Clear'} action={deleteChats} state={deleteCheckMark} />
                    </ul>
                )}
            </div>
        </>
